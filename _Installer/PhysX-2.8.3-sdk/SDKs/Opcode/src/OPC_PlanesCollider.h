...
#define OPC_PLANESCOLLIDER_H
	...
	class OPCODE_API PlanesCollider : public VolumeCollider
	{
		...
		// Internal methods
							...
							void            _Collide(const AABBStacklessCollisionNode* node, const AABBStacklessCollisionNode* const nodes, udword clip_mask);
                            void            _CollideNoPrimitiveTest(const AABBStacklessCollisionNode* node, const AABBStacklessCollisionNode* const nodes, udword clip_mask);
    #endif
    #ifdef OPC_SUPPORT_STACKLESS_TREE_QUANTIZED
							void            _Collide(const AABBStacklessQuantizedNode* node, const AABBStacklessQuantizedNode* const nodes, udword clip_mask);
                            void            _CollideNoPrimitiveTest(const AABBStacklessQuantizedNode* node, const AABBStacklessQuantizedNode* const nodes, udword clip_mask);
    #endif
    #ifdef OPC_SUPPORT_STACKLESS_TREE_NO_LEAF
							void            _Collide(const AABBStacklessNoLeafNode* node, const AABBStacklessNoLeafNode* const nodes, udword clip_mask);
                            void            _CollideNoPrimitiveTest(const AABBStacklessNoLeafNode* node, const AABBStacklessNoLeafNode* const nodes, udword clip_mask);
    #endif
    #ifdef OPC_SUPPORT_STACKLESS_TREE_NO_LEAF_QUANTIZED
							void            _Collide(const AABBStacklessQuantizedNoLeafNode* node, const AABBStacklessQuantizedNoLeafNode* const nodes, udword clip_mask);
                            void            _CollideNoPrimitiveTest(const AABBStacklessQuantizedNoLeafNode* node, const AABBStacklessQuantizedNoLeafNode* const nodes, udword clip_mask);
    #endif
	...
	class OPCODE_API HybridPlanesCollider : public PlanesCollider
	{
		...
		// Mutant box-with-planes collision queries
		inline_				bool			Collide(PlanesCache& cache, const OBB& box, const HybridModel& model, const Matrix4x4* worldb=null, const Matrix4x4* worldm=null)
											{
												Plane PL[6];
