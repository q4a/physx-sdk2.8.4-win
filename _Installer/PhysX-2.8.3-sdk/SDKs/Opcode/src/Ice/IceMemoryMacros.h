...
*/
	...
	#define ICE_ALLOC_MEM(x,y)		GetAllocator()->mallocDebug(x, __FILE__, __LINE__, #x, MEMORY_##y)
#else
	#define ICE_ALLOC_TMP(x)	GetAllocator()->malloc(x, MEMORY_TEMP)
	#define ICE_ALLOC(x)		GetAllocator()->malloc(x, MEMORY_PERSISTENT)
	#define ICE_ALLOC_MEM(x,y)		GetAllocator()->malloc(x, MEMORY_##y)
#endif
	#define ICE_ALLOC_ARRAY(x, y, z)		(x*)ICE_ALLOC_MEM(sizeof(x)*(y), z)

#define	ICE_FREE(x)			if(x)	{ GetAllocator()->free(x); x = null;	}

#ifdef _DEBUG
	#define ICE_NEW_TMP(x)		new(__FILE__, __LINE__, #x, MEMORY_TEMP) x
	#define ICE_NEW(x)			new(__FILE__, __LINE__, #x, MEMORY_##x) x
	#define ICE_NEW_MEM(x,y)			new(__FILE__, __LINE__, #x, MEMORY_##y) x
#else
	#define ICE_NEW_TMP(x)		new(MEMORY_TEMP) x
	#define ICE_NEW(x)			new(MEMORY_##x) x
	#define ICE_NEW_MEM(x,y)			new(MEMORY_##y) x
#endif
