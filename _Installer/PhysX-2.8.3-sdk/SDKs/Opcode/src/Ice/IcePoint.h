...
// Removed since it introduced the nasty "Point T = *Matrix4x4.GetTrans();" bug.......
		...
		//! Adds a vector
		inline_	Point&			Add(const float f[3])					{ x += f[X_];	y += f[Y_];	z += f[Z_];			return *this;	}
		//! Adds vectors
		...
		//! Subtracts a vector
		inline_	Point&			Sub(const float f[3])					{ x -= f[X_];	y -= f[Y_];	z -= f[Z_];			return *this;	}
		//! Subtracts vectors
		...
		//Assignment operator
#ifdef _XBOX
		//An assignment operator is important so the compiler can track variables in registers(otherwise it resorts to memcpy)
		//We probably want this on PC as well?
