...
#define AABB_COMMON_METHODS																											\
	...
	class ICEMATHS_API AABB : public Allocateable
	{
		...
		//! Versions to return a refrence, needed to avoid LHS in SIMD code.
...
//				_prefetch(list+1);	// off by one ?
	...
	class ICEMATHS_API BigAABB
	{
		...
		inline_			BigType		GetMin(udword axis)						const		{ return mMin[axis];						}
		inline_			BigType		GetMax(udword axis)						const		{ return mMax[axis];						}
		inline_			void		GetMin(BigPoint& min)					const		{ min = mMin;								}
		inline_			void		GetMax(BigPoint& max)					const		{ max = mMax;								}
		inline_			void		GetCenter(BigPoint& center)				const		{	center.x = (mMax.x + mMin.x)*0.5;
																							center.y = (mMax.y + mMin.y)*0.5;
																							center.z = (mMax.z + mMin.z)*0.5;}
		inline_			void		GetExtents(BigPoint& extents)			const		{	extents.x = (mMax.x - mMin.x)*0.5;
																							extents.y = (mMax.y - mMin.y)*0.5;
																							extents.z = (mMax.z - mMin.z)*0.5;}

		inline_			BigType		GetCenter(udword axis)					const		{ return (mMax[axis] + mMin[axis])*0.5;		}
		inline_			BigType		GetExtents(udword axis)					const		{ return (mMax[axis] - mMin[axis])*0.5;		}

						void		SetMinMax(const BigPoint& min, const BigPoint& max)		{ mMin = min;		mMax = max;			}
						void		SetCenterExtents(const BigPoint& c, const BigPoint& e)	{ mMin.x = c.x - e.x;
																							mMin.y = c.y - e.y;
																							mMin.z = c.z - e.z;
																							mMax = c + e;		}
						void		SetCenterExtents(const BigPoint& c, const Point& e)	{ mMin.x = c.x - e.x;
																							mMin.y = c.y - e.y;
																							mMin.z = c.z - e.z;
																							mMax.x = c.x + e.x;
																							mMax.y = c.y + e.y;
						...
						void		SetEmpty()											{ BigPoint p(MIN_FLOAT, MIN_FLOAT, MIN_FLOAT);	mMin = -p; mMax = p;}

				BigAABB&			Add(const BigAABB& aabb)
				{
					// Compute new min & max values
					BigPoint Min;	GetMin(Min);
					BigPoint Tmp;	aabb.GetMin(Tmp);
					Min.Min(Tmp);

					BigPoint Max;	GetMax(Max);
					aabb.GetMax(Tmp);
					Max.Max(Tmp);
		...
		inline_			BOOL		Intersect(const BigAABB& a)				const
									{
										if(mMax.x < a.mMin.x
										|| a.mMax.x < mMin.x
										|| mMax.y < a.mMin.y
		...
		inline_			BigAABB&	operator*=(float s)
									{
										BigPoint Center;	GetCenter(Center);
										BigPoint Extents;	GetExtents(Extents);
										SetCenterExtents(Center, Extents * s);
										return *this;
									}
		inline_			BOOL		IsValid()	const
						...
						BigType		MakeCube(BigAABB& cube)		const
									{
										BigPoint Ext;	GetExtents(Ext);
										BigType Max = Ext.Max();

										BigPoint Cnt;	GetCenter(Cnt);
										cube.SetCenterExtents(Cnt, BigPoint(Max, Max, Max));
										return Max;
									}
		inline_			BOOL		ContainsPoint(const BigPoint& p)	const
									{
										if(p.x > GetMax(0) || p.x < GetMin(0)) return FALSE;
										if(p.y > GetMax(1) || p.y < GetMin(1)) return FALSE;
										if(p.z > GetMax(2) || p.z < GetMin(2)) return FALSE;
						...
						bool		IsInside(const BigAABB& box) const
									{
						...
						BigPoint	mMin;			//!< Min point
						BigPoint	mMax;			//!< Max point
	};
