...
//		if(IR(u)&0x80000000 || u>det)					return FALSE;
		...
		if(mStabbedFace.mU < -mGeomEpsilon || mStabbedFace.mU > det + mGeomEpsilon)	return FALSE;
#else
...
#ifdef OPC_GEOM_EPSILON
		if(mStabbedFace.mV < -mGeomEpsilon || (mStabbedFace.mU + mStabbedFace.mV) > det + mGeomEpsilon)	return FALSE;
#else
