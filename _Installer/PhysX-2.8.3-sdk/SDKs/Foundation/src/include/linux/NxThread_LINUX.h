#ifndef NX_FOUNDATION_NXTHREAD_LINUX_H
#define NX_FOUNDATION_NXTHREAD_LINUX_H
/*----------------------------------------------------------------------------*\
|
...
{
	#if !defined(__flash__) && !defined(__webgl__)
		NX_COMPILE_TIME_ASSERT(sizeof(NxThreadId)==sizeof(pthread_t));
		return (NxThreadId)pthread_self();
	#else
		return NULL;
	#endif
}
