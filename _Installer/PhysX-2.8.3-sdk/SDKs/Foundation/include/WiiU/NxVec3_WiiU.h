#ifndef NX_FOUNDATION_NXVEC3_WIIU
#define NX_FOUNDATION_NXVEC3_WIIU
/*----------------------------------------------------------------------------*\
|
...
NX_INLINE NxReal NxVec3::dot(const NxVec3 &v) const
	{
	return VECDotProduct(&m_v, &v.m_v);
	}
...
NX_INLINE NxVec3 NxVec3::operator -() const
	{
		NxVec3 retvec;
		register const NxVec3* p = this;
		register NxVec3* pOut = &retvec;
...
NX_INLINE NxVec3 NxVec3::operator -(const NxVec3 & v) const
	{
	NxVec3 temp;
	VECSubtract(&m_v, &v.m_v, &temp.m_v);
	return temp;
	}
