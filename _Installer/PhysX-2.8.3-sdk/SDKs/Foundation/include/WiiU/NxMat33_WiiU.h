#ifndef NX_FOUNDATION_NxMat33T_WIIU
#define NX_FOUNDATION_NxMat33T_WIIU
/*----------------------------------------------------------------------------*\
|
...
NX_INLINE void NxMat33::fromQuat(const NxQuat & q)
	{
	Mtx m;
	MTXQuat( m, (Quaternion*)&q.x);
	setRowMajorStride4(m);
	}
...
NX_INLINE void NxMat33::toQuat(NxQuat & q) const					// set the NxQuat from a rotation matrix
    ...
    if(tr >= 0)
		{
		s = (NxReal)NxMath::recipSqrt(tr + 1);
		q.w = NxReal(0.5) / s;
		s = NxReal(0.5) * s;
		...
		switch (i)
			{
			case 0:
				s = (NxReal)NxMath::recipSqrt((data.s._11 - (data.s._22 + data.s._33)) + 1);
				q.x = NxReal(0.5) / s;
				s = NxReal(0.5) * s;
			...
			case 1:
				...
				s = (NxReal)NxMath::recipSqrt((data.s._22 - (data.s._33 + data.s._11)) + 1);
				q.y = NxReal(0.5) / s;
				s = NxReal(0.5) * s;
			...
			case 2:
				...
				s = (NxReal)NxMath::recipSqrt((data.s._33 - (data.s._11 + data.s._22)) + 1);
				q.z = NxReal(0.5) / s;
				s = NxReal(0.5) * s;
